...
if [ "$1" = "--uninstall" ]
then
    if grep -q "wallpaper-gdm.png" /usr/share/gnome-shell/gnome-shell-theme.gresource
    then
...
then
    case "$2" in
	0) image_parameters="background-repeat: no-repeat;";;
	1) image_parameters="background-repeat: no-repeat;";;
	2) image_parameters="background-repeat: no-repeat;background-size: cover;";;
	3) image_parameters="background-size: 3840px 2160px;";;
	4) image_parameters="background-size: 3840px 2160px;background-repeat: no-repeat;";;
	5) image_parameters="background-position: 0 0;background-size: 3840px 2160px;background-repeat: no-repeat;";;
...
for res_file in $(gresource list /usr/share/gnome-shell/gnome-shell-theme.gresource)
do
  mkdir -p "$(dirname "$workdir$res_file")"

  if [ "$res_file" != "/org/gnome/shell/theme/wallpaper-gdm.png" ]
  then
